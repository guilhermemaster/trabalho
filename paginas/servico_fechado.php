...
<?php	
 ...
 <form action="servico_fechado.php" method="post" >
   ...
   <table border="1"><tr><th>NÚMERO DA ORDEM DE SERVIÇO</th> <th>NOME DO CLIENTE</th> <th>ENDEREÇO</th> <th>UF</th><th>CEP</th><th>CIDADE</th>
   	<th>BAIRRO</th><th>TELEFONE (1)</th><th>TELEFONE (2)</th><th>DATA DO SERVIÇO</th><th>VALOR</th><th>DESCRIÇÃO DO SERVIÇO</th><th>EXCLUSÃO</th></tr>
 
<?php

$procura_filme=strtoupper($_POST['nome_filme_procura']);
	if (empty($procura_filme)){ 
			$sql="select * from ordem_servico where status_servico ='0'";
					}
						else{
							$sql="select * from ordem_servico where status_servico ='0' and '%".$procura_filme."%'";
								}
					...
					if(@mysql_num_rows($resultado1)==0) die ("Nenhum registro encontrado");
						...
						print "<td><form action=\"servico_fechado.php\" method=\"post\" >
								<input name=\"filmes_baixados\" type=\"submit\" value=\"EXCLUIR\" />
								<input name=\"verificar\" type=\"hidden\" value=\"deleta\"  />
						...
						if($_POST['verificar']=='deleta'){
							$sql="delete from ordem_servico where idservico=".$_POST['id'];
							executa_sql($sql);
							print "<meta http-equiv=\"refresh\" content=\"0;URL=servico_fechado.php\">";
